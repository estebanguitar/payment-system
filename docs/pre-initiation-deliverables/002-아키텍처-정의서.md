...
* **암호화 알고리즘**: **AES-256-GCM** (또는 CBC + PKCS5Padding)
* **키 관리**: Secret Key는 소스코드나 DB에 저장하지 않고, `application.yml`의 `payment.security.encryption-key` 또는 환경 변수(`PAYMENT_ENCRYPTION_SECRET`)를 통해 실행 타임에 주입.
