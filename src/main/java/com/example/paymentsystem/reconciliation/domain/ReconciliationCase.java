...
public class ReconciliationCase {
  ...
  @Column(name = "case_key", nullable = false, unique = true, length = 200)
  private String caseKey;

  @Column(name = "latest_run_id", nullable = false)
  ...
  public static ReconciliationCase open(ReconciliationFinding f, Long runId, LocalDateTime now) {
    ...
    c.caseKey = f.caseKey();
    c.latestRunId = runId;
    c.mismatchType = f.type();
    c.severity = f.severity();
    c.status = CaseStatus.OPEN;
    c.customerId = f.customerId();
    c.walletId = f.walletId();
    c.paymentId = f.paymentId();
    c.cancelId = f.cancelId();
    c.outboxId = f.outboxId();
    c.expectedValue = f.expectedValue();
    c.actualValue = f.actualValue();
    c.evidence = f.evidence();
    c.firstDetectedAt = now;
    c.lastDetectedAt = now;
    return c;
  }
  ...
  public void redetect(ReconciliationFinding f, Long runId, LocalDateTime now) {
    ...
    evidence = f.evidence();
    lastDetectedAt = now;
    if (status == CaseStatus.RESOLVED) status = CaseStatus.ACTION_REQUIRED;
    resolvedAt = null;
  }

  /** 허용된 수동 조치를 적용하고 변경 전 상태를 반환한다. */
  public CaseStatus apply(ActionType action) {
    CaseStatus before = status;
    status =
        switch (action) {
          case START_INVESTIGATION -> CaseStatus.INVESTIGATING;
