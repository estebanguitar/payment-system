...
public class OutboxWorkerScheduler {
    ...
    @Scheduled(
            fixedDelayString = "${payment.outbox.fixed-delay:60000}",
            initialDelayString = "${payment.outbox.initial-delay:${payment.outbox.fixed-delay:60000}}")
    public void process() {
        metrics.heartbeat();
    ...
    private void processClaimed(Long id) {
        try {
            ...
            if (outbox == null || outbox.getStatus() != OutboxStatus.DEAD_LETTER) {
                metrics.retry();
            }
